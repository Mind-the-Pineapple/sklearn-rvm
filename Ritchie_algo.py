...
class BaseRVM(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iter):
            ...
            self.alpha_ = self.gamma/(self.m_ ** 2 + np.zeros(self.m_.shape)) 
            if np.isnan(self.alpha_[0]):
                break
            if not self.beta_fixed:
                ...
                print('m = {}'.format(np.isnan(self.m_).any()))
                print('gamma = {}'.format(self.gamma))
                print('beta_num = {}'.format((n_samples - np.sum(self.gamma))))
                print('beta_ord = {}'.format(np.sum((y - np.dot(self.phi, self.m_)) ** 2)))
...
class RVR(BaseRVM, RegressorMixin):
    ...
    def _posterior(self):
        ...
        try:
            ...
            raise('Error on inv')
        if np.isnan(self.sigma_).any():
            print('i_s = {}'.format(i_s))
            print('alpha_ = {}'.format(self.alpha_))
    ...
    def predict(self, X, eval_MSE=False):
        ...
        if self.kernel != 'precomputed':
            phi = self._apply_kernel(X, self.relevance_vectors_)
        else:
            phi = X[self.relevance_, :].T
            if self.bias_used:
                phi = np.append(phi, np.ones((phi.shape[0], 1)), axis=1)
